...
function genResultTable(request: RequestData) {
  // ถ้ามี date อยู่เเล้ว ให้เพิ่มข้อมูลใน date
  if (result[request.date]) {
    console.log("มี date นี้เเล้ว",request.date);
    // result[request.date] = {
    //   ...result[request.date],
    //     man_waiting_review: request.request_process_id,
    //   man_approved: request.current_state_id
    // }
    console.log("พวกค่า waiting approved ในวันที่นั้น");
    // console.log("request.date", result[request.date])
    console.log("resultaftercondition", result);
  } else {
    // create new result
    console.log("ยังไม่มี date นี้",request.date);
    let test = result[request.date]
